...
cd ../local/share &&\
find .* -maxdepth 0 -name '.[^.]*' | while read file; do rm "$HOME/.local/share/$file"; ln -s "`pwd`/$file" "$HOME/.local/share/$file"; done
find * -maxdepth 0 | while read file; do rm "$HOME/.local/share/$file"; ln -s "`pwd`/$file" "$HOME/.local/share/$file"; done
